  ...
  <div class="sms-page">
    ...
    <!--v-model="input3">-->
      ...
      <div class="retry-choose">
        <el-button type="danger"  plain @click="openmsg"> Retry Choose</el-button>
      ...
      <el-table-column width="90px" align="center" >
        ...
        <template slot-scope="props">
          ...
          <div v-else-if="props.row.sendSms === 0">
            <el-button type="danger" icon="el-icon-refresh-left" @click="msgoneretry" circle></el-button>
          </div>
          <div v-else>
          </div>
        </template>
...
export default {
  ...
  computed: {
    ...
    allSMSSelected() {
        return this.isSelected.length === this.dataList.length;
      },
  }, 
  methods:{
    ...
    async getdata(){
      ...
      try {
          const response = await axios.get(`http://localhost:7777/api/notification/database?page=${page}&pageSize=${pageSize}&sortOrder=${sortOrder}`)
            // ตรวจสอบค่า response.data ว่าเป็นอาเรย์หรือไม่
          ...
          if (Array.isArray(response.data.content)) {
            ...
            this.dataList = []; // หรือกำหนดให้เป็นอาเรย์ว่างเปล่าหากไม่ใช่อาเรย์
          }
    ...
    async searchData() {
      ...
      const sortOrder = this.sortOrder;
        ...
        if (this.selectsentsms === 'SMS_SUCCESS') {
          console.log(this.selectSMSOption)
        ...
        if (this.searchInput) {
          const searchString = encodeURIComponent(this.searchInput);
      ...
      async retryChoose() {
        const selectedDataToSend = this.dataList.filter(row => row.isSelected && row.sendSms === 0).map(row => row.id);
    console.log(selectedDataToSend);
      },
    async retrysms(){
      const selectedIDs = this.dataList.filter(item => item.isSelected).map(item => item.id) 
      try{
      const response = await axios.post(`http://localhost:7777/api/notification/resend?id=${selectedIDs}`)
      console.log(response)
      this.getdata()
      }catch (error){
    ...
    async openmsg() {
      if (!this.dataList.some(item => item.isSelected)) {
        this.$message({
          type: 'error',
    ...
    selectAllRows(value) {
    // Set isSelected property for all rows with sendSms === 1 to the value of selectAll
        const rowsWithSendSms0 = this.dataList.filter(row => row.sendSms === 0);
      rowsWithSendSms0.forEach(row => {
